...
void processData(char* buf) {
  ...
  if (!strcmp(buf, "flash")) {
    fprintf(stderr, "0x08000000: %x\r\n", *((int *) (0x08000000)));
    fprintf(stderr, "0x08020000: %x\r\n", *((int *) (0x08020000)));
    HAL_FLASH_Unlock();
    __HAL_FLASH_CLEAR_FLAG(
            FLASH_FLAG_EOP | FLASH_FLAG_OPERR | FLASH_FLAG_WRPERR | FLASH_FLAG_PGSERR | FLASH_FLAG_PGSERR);
    FLASH_Erase_Sector(FLASH_SECTOR_1, FLASH_BANK_1, VOLTAGE_RANGE_3);
    fprintf(stderr, "0x08000000: %x\r\n", *((int *) (0x08000000)));
    fprintf(stderr, "0x08020000: %x\r\n", *((int *) (0x08020000)));
    for (int i = 0; i < 32000; ++i) {
      size_t fraddr = 0x08000000 + 4 * i;
      HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, fraddr + 0x20000, fraddr);
    }
    HAL_FLASH_Lock();
    fprintf(stderr, "0x08000000: %x\r\n", *((int *) (0x08000000)));
    fprintf(stderr, "0x08020000: %x\r\n", *((int *) (0x08020000)));
  ...
  if (!strncmp(buf,"adxl355",7)) {
	...
	bool ledCmd = !(strcmp(cmd, "ledEnable") && strcmp(cmd, "ledDisable"));

	if (idStr == NULL || cmd == NULL || !ledCmd) {
		printf("Unknown command: %s\r\n", oldBuf);
		return;
	}
    unsigned id = atol(idStr);
	if (id < 1 || id > 3) {
	    printf("Bad ID: %d\r\n", id);
		return;
	}
	if (!strcmp(cmd, "ledEnable")) {
      if (id == 1) {
	    BSP_LED_On(LED1);
	  } else if (id == 2) {
	  	BSP_LED_On(LED2);
...
int main(void)
{
  ...
  while (1) {
    ...
    if (*rptr == '\r' || (rptr - buf) == 255) {
      *rptr = '\0';
      fprintf(stderr, "%d Got command: %s\r\n", ccount++, (char*)buf);
